...
\begin{quote}对于一个存储 \(n\) 个 \((1 + \Theta(1)) \log n\)-bit key/value pairs 的 hash table，它究竟能提供多强的 probabilistic guarantee？\cite[Abstract and Sec.~1]{kuszmaul2022}\end{quote}
...
作者给出的回答非常鲜明：很多限制并不是 hash table 这个目标的内在限制，而是“必须通过 hash functions 来实现它”这一范式的限制\cite[Sec.~1]{kuszmaul2022}。
...
其中 \(\epsilon > 0\) 是任意正常数\cite[Theorem~2]{kuszmaul2022}。
...
那么就会逼近 deterministic constant-time dictionary 这个更大的 open question。因此这个结果在某种意义上已经接近理论边界\cite[Lemma~3]{kuszmaul2022}。
...
random bits\cite[Theorem~5]{kuszmaul2022}；
...
这个结果不是随机位数量上的新纪录：论文指出，Dietzfelbinger、Gil、Matias 和 Pippenger 的工作已经能在当前 key-size setting 下用 \(O(\log n)\) random bits 达到同级别概率保证\cite[Sec.~5]{kuszmaul2022}\cite{dietzfelbinger1992}。它的价值主要在于不直接依赖在线 hash-function evaluation 的结构路线、对 gradually-increasing-independence 的初始化阶段使用，以及它后来作为 succinct transformation 的关键组件。

\subsection{succinct transformation}

在上述两条结果之外，论文还证明了一个更通用的 succinctness 结果：

\begin{quote}在 Theorem 7 的前提条件下，符合规定空间形式的 constant-time dictionary，可以被变换成 succinct constant-time dictionary，同时近似保留它的 random-bit usage 与 failure probability。\cite[Theorem~7]{kuszmaul2022}\end{quote}

因此，前面两条主结果都能被推进到 succinct setting 中。这一步把论文从“两个具体构造”提升成了“一个新的通用结构变换框架”。

\section{模型边界与结果口径}

前面的主要结果需要放在论文规定的模型中理解。首先，Section 2--5 主要讨论至多存储 \(n\) 个 \(\Theta(\log n)\)-bit key/value pairs 的非 succinct dictionary，并默认机器字长为 \(\Theta(\log n)\)；更大字长的情形放在 Appendix A，不能直接套用前面的结论\cite[Sec.~2 and Appendix~A]{kuszmaul2022}。

其次，论文中的 \texttt{failure} 主要不是正确性错误，而是某次操作耗时超过常数时间。在 succinct dictionary 中，failure 还可能表示结构无法满足规定的空间界。因此，本文后面所说的“失败概率”应理解为时间或空间保证失败概率，而不是查询返回错误答案的概率\cite[Secs.~2 and 6]{kuszmaul2022}。

论文还采用了几个重要的分析约定：其概率界针对固定操作序列分析，不覆盖根据结构内部随机性自适应选择输入的 adversary；通过周期性 rebuild，把一个分析阶段限制在 \(O(n)\) 次操作内；概率是针对任意给定操作的 per-operation 概率，不是整个无限生命周期永不失败的概率；一旦阶段内发生 failure，后续操作可以退化为较慢实现\cite[Sec.~2]{kuszmaul2022}。
...
级别的随机位，并把失败概率参数变为 \(p(n/\log\log n)\)\cite[Theorem~7]{kuszmaul2022}。

\section{相关工作与课程联系}

\subsection{相关工作的主线}

传统 hash table 通过 hash function 把 key 映射到数组位置，universal hashing、perfect hashing、cuckoo hashing 等路线都把冲突控制交给显式随机映射，再结合重建、多个候选位置或二级结构保证性能\cite{carter1979,fks1982,pagh2004}。本文并不是否定这些技术，而是追问：随机性是否可以直接嵌入数据结构布局，从而绕开“既要足够强、又要常数时间求值”的 hash-function family 瓶颈\cite[Secs.~1 and 3]{kuszmaul2022}。

论文直接依赖或重点对照的工作包括：Fredman--Komlós--Szemerédi 的 perfect hashing\cite{fks1982}；Fredman--Willard 与 Pătraşcu--Thorup 的 fusion/dynamic fusion node\cite{fredman1990,patrascu2014}；Dietzfelbinger 等人的 polynomial hash functions\cite{dietzfelbinger1992}；Raman--Rao 的 succinct dynamic dictionaries\cite{raman2003}；Celis 等人与 Meka 等人的 load-balancing families\cite{celis2011,meka2014}；以及 Pătraşcu--Thorup 关于 simple tabulation hashing 的研究\cite{patrascu2012}。
...
论文发表后的相关方向应谨慎表述为相邻研究，而不是本文的直接定理级后续。\texttt{Modern Hashing Made Simple} 研究如何用标准工具组合出兼具高概率常数时间和紧凑空间的结构\cite{bender2024}；\texttt{Optimal Non-Oblivious Open Addressing} 则研究满负载、动态空间和高概率常数时间的 open-addressed table\cite{bender2025}。它们与本文共享重新审视传统 hash-table 约束的问题意识，但技术模型不同，并不等同于对 rotated trie 或 Theorem 7 的直接改进。
...
但它的致命问题是空间太差。内部节点可能有 \(\Theta(n)\) 个，每个节点又维护大小为 \(n\) 的数组，因此总空间可能达到 \(\Omega(n^2)\)\cite[Sec.~3]{kuszmaul2022}。
...
统一数组中的每个位置由 dynamic fusion node 承载，只要每个位置的负载不超过 \(\operatorname{polylog}(n)\)，结构就仍然可以常数时间操作\cite[Proposition~1]{kuszmaul2022}。
...
这一步相当于把随机性分散到更多局部结构中，然后作者不再逐个 bin 地分析，而是直接分析 overflow balls 总数 \(q\)，并用 McDiarmid’s inequality 证明\cite[Sec.~4]{kuszmaul2022}：
...
更妙的是，作者没有把后者当作在线 hash function，而是只把它当成 internal node 初始化时的一次性 pseudo-random number generator。这样就绕开了它求值慢的缺点\cite[Sec.~5]{kuszmaul2022}\cite{celis2011,meka2014}。
...
Section 6 的技术味道与前面不同。这里作者没有再直接发明一个新的 hash-table-like structure，而是先做一个 Raman--Rao reduction，把 succinct dictionary 问题转成 \texttt{\detokenize{many-sets problem}}\cite[Sec.~6.1]{kuszmaul2022}\cite{raman2003}。
...
正文主结果主要处理 key 和 value 均为 \(\Theta(\log n)\) bits 的情形。附录里作者讨论了 super-polynomial universe 的处理方式，但整体路线已经更依赖额外的 universe reduction 技术\cite[Appendix~A]{kuszmaul2022}。
...
作者其实是在不断靠近这个更大的 open question，但并没有真正解决它。恰恰因为这篇论文把 randomized 边界推进得更靠前，反而让这个大问题显得更尖锐了\cite[Sec.~1 and Lemma~3]{kuszmaul2022}。
...
本项目为单人完成，成员为丁宣中（学号：241880267）。本人负责论文选题与材料整理、各章节阅读笔记、核心结构与证明思路梳理、相关工作调研、论文评价、综述撰写与排版、内容校核及版本迭代。上述工作按阶段提交至仓库，具体形成过程可由默认分支的笔记文件与 Git 提交历史追溯。

\section{AI 使用说明}

本项目在两个明确环节使用了 AI 辅助。第一，基于论文原文生成中英逐段对照材料，并协助完成相应的结构整理、翻译和排版；第二，在最终 review 的引用整理阶段，辅助定位需要引用的关键论断，核对原论文中的章节、定理与引理，整理参考文献元数据，并完成 LaTeX 引用结构和编译结果的检查。

AI 没有替代原论文成为学术结论的依据，也没有独立提出本文所述的算法、定理或评价。综述中的技术内容和引用均以论文原文、原始相关工作及可核验的出版信息为准，本人负责理解并复查 AI 输出。更完整的输入材料、参与范围、约束和输出记录见仓库的 \texttt{ai-log/} 目录。

\section{参考文献与调研边界}

正文对原论文结论的引用精确到相应章节、定理或引理；相关工作尽量引用原始论文。论文发表后的两项相邻研究只用于说明主题脉络，不把它们宣称为本文的直接延伸。

\begin{thebibliography}{99}

\bibitem{kuszmaul2022}
William Kuszmaul.
\newblock \textit{A Hash Table Without Hash Functions, and How to Get the Most Out of Your Random Bits}.
\newblock arXiv:2209.06038, 2022.

\bibitem{carter1979}
J. Lawrence Carter and Mark N. Wegman.
\newblock Universal Classes of Hash Functions.
\newblock \textit{Journal of Computer and System Sciences}, 18(2):143--154, 1979.

\bibitem{fks1982}
Michael L. Fredman, János Komlós, and Endre Szemerédi.
\newblock Storing a Sparse Table with $O(1)$ Worst Case Access Time.
\newblock In \textit{Proceedings of FOCS}, pages 165--169, 1982.

\bibitem{pagh2004}
Rasmus Pagh and Flemming Friche Rodler.
\newblock Cuckoo Hashing.
\newblock \textit{Journal of Algorithms}, 51(2):122--144, 2004.

\bibitem{fredman1990}
Michael L. Fredman and Dan E. Willard.
\newblock Blasting Through the Information Theoretic Barrier with Fusion Trees.
\newblock In \textit{Proceedings of STOC}, pages 1--7, 1990.

\bibitem{patrascu2014}
Mihai Pătraşcu and Mikkel Thorup.
\newblock Dynamic Integer Sets with Optimal Rank, Select, and Predecessor Search.
\newblock In \textit{Proceedings of FOCS}, pages 166--175, 2014.

\bibitem{dietzfelbinger1992}
Martin Dietzfelbinger, Joseph Gil, Yossi Matias, and Nicholas Pippenger.
\newblock Polynomial Hash Functions Are Reliable.
\newblock In \textit{Proceedings of ICALP}, pages 235--246, 1992.

\bibitem{raman2003}
Rajeev Raman and Satti Srinivasa Rao.
\newblock Succinct Dynamic Dictionaries and Trees.
\newblock In \textit{Proceedings of ICALP}, pages 357--368, 2003.

\bibitem{celis2011}
L. Elisa Celis, Omer Reingold, Gil Segev, and Udi Wieder.
\newblock Balls and Bins: Smaller Hash Families and Faster Evaluation.
\newblock In \textit{Proceedings of FOCS}, pages 599--608, 2011.

\bibitem{meka2014}
Raghu Meka, Omer Reingold, Guy N. Rothblum, and Ron D. Rothblum.
\newblock Fast Pseudorandomness for Independence and Load Balancing.
\newblock In \textit{Proceedings of ICALP}, pages 859--870, 2014.

\bibitem{patrascu2012}
Mihai Pătraşcu and Mikkel Thorup.
\newblock The Power of Simple Tabulation Hashing.
\newblock \textit{Journal of the ACM}, 59(3), Article 14, 2012.

\bibitem{bender2024}
Michael A. Bender, Martín Farach-Colton, John Kuszmaul, and William Kuszmaul.
\newblock Modern Hashing Made Simple.
\newblock In \textit{Proceedings of SOSA}, pages 363--373, 2024.
\newblock \url{https://doi.org/10.1137/1.9781611977936.33}.

\bibitem{bender2025}
Michael A. Bender, William Kuszmaul, and Renfei Zhou.
\newblock Optimal Non-Oblivious Open Addressing.
\newblock In \textit{Proceedings of STOC}, pages 268--277, 2025.
\newblock \url{https://doi.org/10.1145/3717823.3718215}.
